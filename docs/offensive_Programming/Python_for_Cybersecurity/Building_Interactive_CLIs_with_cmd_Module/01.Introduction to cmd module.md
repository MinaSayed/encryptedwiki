...
## What is the line-orinted command interpretor:
A command interpreter is a program that:
-Prints a prompt (like >>> or myapp> ).
-Waits for you to type a line of text.
-Runs some code based on that line.
-Then loops back to the prompt again.
​

This is what cmd gives you: a ready‑made loop that:
-Shows a prompt.
-Reads a line.
-Parses it into a command name and arguments.
-Calls the right method on your class.
-Repeats until you tell it to stop.
​
So cmd is not the shell itself; it’s the engine that drives an interactive line‑oriented shell.
...
In standard programming, a script runs from the top line to the bottom line and then stops.
To build a tool that feels like a real "app," the program must not stop. It must enter a Loop.

Think of a loop like a guard at a gate. The guard does not go home after the first person walks through.
The guard waits, asks for a "command" or "ID," processes it, and then goes back to waiting.
...
The cmd module automates this loop so you don't have to write it yourself.
But if you don't understand that your program is now "trapped" in a circle, you will never control it.
...
In the Eval phase, the danger isn't just "doing an action." The danger is untrusted input.
If your evaluation logic is weak, a user could enter something that crashes the process,
leaks data, or bypasses security. You are the gatekeeper; if your logic is flawed, the "gate" is useless.
