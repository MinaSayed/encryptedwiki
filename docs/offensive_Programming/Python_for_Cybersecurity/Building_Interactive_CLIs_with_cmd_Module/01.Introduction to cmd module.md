...
import cmd
```
### Creating a custom class child inherits form the parent class (cmd.Cmd)
After importing the cmd module there is a class called cmd.Cmd this class is very improtant class inside the cmd module
it Knows how to loop, how to handle help menus, and how to find commands.

So we create an instance class form the parent class and inherit form the parent all the powers it has.
After creating the child class that inherits form the parent class you can define specific command you want.
...
When you write class MyShell(cmd.Cmd):, you are telling Python: "Give MyShell all the powers of cmd.Cmd
but I will add my own flavor to it.
